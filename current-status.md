...
- Next progress report due: 2026-08-29 06:12 ET.
...
- The exact current Ubuntu/WSL CI sequence passes in an isolated `/tmp` environment at 306 tests and 87% branch coverage, including scoped policy wakeups, drilldown validation, controlled SQLite failures, distribution verification, final-start authority revocation, typed identity, nested DTO validation, and migration `0012`; the shared Windows `.venv` was untouched.
...
- CI now verifies distribution contents on Windows and Ubuntu after every build; the source archive has 72 intended entries with workspace audit surfaces excluded, and the wheel has all 54 required runtime entries/assets.
