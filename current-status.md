...
- The exact current Ubuntu/WSL CI sequence passes in an isolated `/tmp` environment at 305 tests and 87% branch coverage, including drilldown return validation, controlled SQLite failures, distribution verification, final-start authority revocation, typed identity, nested DTO validation, and migration `0011`; the shared Windows `.venv` was untouched.
