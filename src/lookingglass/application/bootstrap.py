...
class SshBootstrapResult:
    """IDs created or reused by :meth:`SystemBootstrapService.configure_ssh_host`."""

    system: SystemRecord
    connection_binding_id: str
    root_object_id: str
    root_scope: ConfiguredScopeRecord
    capability_binding_ids: tuple[str, ...]
...
_SSH_CAPABILITIES: dict[str, tuple[tuple[TargetKind, ...], tuple[str, ...], tuple[str, ...]]] = {
    ...
    "ssh.fs.metadata.read": (
        (TargetKind.OBJECT,),
        ("metadata",),
        ("ssh.fs.folder", "ssh.fs.file"),
    ),
}


def _ssh_coverage_policies(
    capability_key: str, target_kinds: tuple[TargetKind, ...]
) -> tuple[CapabilityCoveragePolicy, ...]:
...
class SystemBootstrapService:
    ...
    def configure_ssh_host(
        ...
        *,
        ...
        enabled_capability_keys: Iterable[str] = ("ssh.fs.children.read",),
        non_secret_settings: Mapping[str, JSONValue] | None = None,
        system_id: str | None = None,
        connection_binding_id: str | None = None,
        ...
        """Seed a remote directory root as a canonical unknown folder and configured scope.

        ``host_alias`` and ``authority_fingerprint`` are bounded references to
        existing local OpenSSH configuration and a pinned host route. No secret
        or key material ever enters the binding.
        """
        host_alias = validate_ssh_host_alias(host_alias, "host_alias")
        authority_fingerprint = validate_authority_fingerprint(
            authority_fingerprint, "authority_fingerprint"
        )
        require_text(path_root, "path_root", max_length=4096)
        capability_keys = tuple(dict.fromkeys(enabled_capability_keys))
        unknown_capabilities = set(capability_keys) - set(_SSH_CAPABILITIES)
        if unknown_capabilities:
            raise ValueError(f"unsupported SSH capability {sorted(unknown_capabilities)!r}")
        reserved = {"host_alias", "authority_fingerprint", "path_root"}
        extra_settings = set(non_secret_settings or ()) & reserved
        if extra_settings:
            raise ValueError(
                f"non_secret_settings cannot override reserved settings: {sorted(extra_settings)!r}"
            )
        ...
        binding_id = connection_binding_id or str(
            uuid5(NAMESPACE_URL, f"ssh-binding:{system.system_id}")
        )
        ...
        root = self.store.upsert_object(
            RemoteObject(
                object_id=root_object_id or str(uuid4()),
                system_id=system.system_id,
                object_type="folder",
                object_type_version="1",
                source_kind="ssh.fs.folder",
                external_key=f"ssh:{path_root}",
        ...
        configured_scope = self.store.create_configured_scope(
            ...
            or str(uuid5(NAMESPACE_URL, f"ssh-host-root:{system.system_id}:{path_root}")),
            system_id=system.system_id,
            object_id=root.object_id,
            object_type="folder",
            display_name=path_root,
        ...
        capability_ids: list[str] = []
        for capability_key in capability_keys:
            target_kinds, produced_facets, target_source_kinds = _SSH_CAPABILITIES[capability_key]
            capability_id = str(
                uuid5(NAMESPACE_URL, f"ssh-capability:{binding_id}:{capability_key}")
            )
            self.store.upsert_capability_binding(
                CapabilityBinding(
                    capability_binding_id=capability_id,
                    ...
                    selection_priority=100,
                    collateral_effects=(
                        "remote authentication record",
                        "remote command execution",
                    ),
                    mitigations=(
                        "fixed registered SSH command mapping",
                        "configured path allowlist",
                    ),
                    coverage_policies=_ssh_coverage_policies(capability_key, target_kinds),
                ),
                now=timestamp,
            )
            capability_ids.append(capability_id)
