...
SELECT * FROM systems
WHERE lookingglass_read_is_uuid(system_id) = 1
  AND lookingglass_read_is_text(display_name, 1024) = 1
  AND lookingglass_read_is_contract_key(system_kind) = 1
  AND enabled IN (0, 1)
  AND lookingglass_read_is_timestamp(record_created_at) = 1
  AND lookingglass_read_is_timestamp(record_updated_at) = 1
  AND lookingglass_read_timestamp_order_is_valid(record_created_at, record_updated_at) = 1;

CREATE VIEW readable_remote_objects AS
SELECT * FROM remote_objects
WHERE lookingglass_read_is_uuid(object_id) = 1
  AND lookingglass_read_is_uuid(system_id) = 1
  AND lookingglass_read_is_contract_key(object_type) = 1
  AND lookingglass_read_is_text(object_type_version, 32) = 1
  AND lookingglass_read_is_contract_key(source_kind) = 1
  AND lookingglass_read_is_text(external_key, 4096) = 1
  AND lookingglass_read_is_text(display_name, 1024) = 1
  AND presence IN ('unknown', 'present', 'absent')
  AND lookingglass_read_is_timestamp(first_seen_at) = 1
  AND (last_seen_at IS NULL OR lookingglass_read_is_timestamp(last_seen_at) = 1)
  AND (
      last_seen_at IS NULL
      OR lookingglass_read_timestamp_order_is_valid(first_seen_at, last_seen_at) = 1
  );

CREATE VIEW readable_facets AS
SELECT * FROM facets
WHERE lookingglass_read_is_uuid(object_id) = 1
  AND lookingglass_read_is_contract_key(facet) = 1
  AND lookingglass_read_is_text(facet_version, 32) = 1
  AND knowledge IN ('unknown', 'known', 'unsupported')
  AND lookingglass_read_is_json_object(payload_json) = 1
  AND (observed_at IS NULL OR lookingglass_read_is_timestamp(observed_at) = 1)
  AND lookingglass_read_is_timestamp(state_changed_at) = 1
  AND (
      supporting_observation_id IS NULL
      OR lookingglass_read_is_uuid(supporting_observation_id) = 1
  )
  AND (source_revision IS NULL OR lookingglass_read_is_text(source_revision, 512) = 1);

CREATE VIEW readable_relationships AS
SELECT * FROM relationships
WHERE lookingglass_read_is_uuid(relationship_id) = 1
  AND lookingglass_read_is_uuid(system_id) = 1
  AND lookingglass_read_is_uuid(subject_id) = 1
  AND lookingglass_read_is_contract_key(predicate) = 1
  AND lookingglass_read_is_uuid(object_id) = 1
  AND presence IN ('unknown', 'present', 'absent')
  AND lookingglass_read_is_timestamp(observed_at) = 1
  AND lookingglass_read_is_uuid(supporting_observation_id) = 1
  AND (
      (
          EXISTS (
...
WHERE lookingglass_read_is_uuid(attempt_id) = 1
  AND lookingglass_read_is_uuid(action_id) = 1
  AND typeof(ordinal) = 'integer'
  AND ordinal > 0
  AND lookingglass_read_is_timestamp(started_at) = 1
  AND (ended_at IS NULL OR lookingglass_read_is_timestamp(ended_at) = 1)
  AND (retry_at IS NULL OR lookingglass_read_is_timestamp(retry_at) = 1);
...
WHERE lookingglass_read_is_uuid(system_id) = 1
  ...
  AND lookingglass_read_is_timestamp(occurred_at) = 1;
...
SELECT
    ...
    severity,
    alertable,
    CASE WHEN system_id IS NULL OR lookingglass_read_is_uuid(system_id) = 1
         THEN system_id ELSE NULL END AS system_id,
    intent_scope_id,
    CASE
        WHEN action_id IS NULL THEN NULL
        WHEN lookingglass_read_is_uuid(action_id) = 1
         AND EXISTS (
    ...
    END AS action_id,
    CASE WHEN attempt_id IS NULL OR lookingglass_read_is_uuid(attempt_id) = 1
         THEN attempt_id ELSE NULL END AS attempt_id,
    error_class,
    redacted_summary,
    occurred_at
FROM operational_events
WHERE lookingglass_read_is_uuid(event_id) = 1
  AND lookingglass_read_is_contract_key(event_type) = 1
  AND lookingglass_read_is_contract_key(severity) = 1
  AND alertable IN (0, 1)
  AND typeof(redacted_summary) = 'text'
  AND lookingglass_read_is_timestamp(occurred_at) = 1;

UPDATE adapter_actions
SET record_created_at = lookingglass_canonicalize_timestamp(record_created_at)
WHERE lookingglass_canonicalize_timestamp(record_created_at) IS NOT NULL;

UPDATE operational_events
SET occurred_at = lookingglass_canonicalize_timestamp(occurred_at)
WHERE lookingglass_canonicalize_timestamp(occurred_at) IS NOT NULL;
