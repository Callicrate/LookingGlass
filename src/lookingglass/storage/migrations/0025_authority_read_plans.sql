...
WHERE rookery_read_is_uuid(relationship.relationship_id) = 1
  AND rookery_read_is_uuid(relationship.system_id) = 1
  AND rookery_read_is_uuid(projection.subject_id) = 1
  AND rookery_read_is_contract_key(projection.predicate) = 1
  AND rookery_read_is_uuid(projection.object_id) = 1
  AND rookery_read_is_contract_key(projection.object_type) = 1
  AND projection.presence IN ('unknown', 'present', 'absent')
  AND rookery_read_is_timestamp(relationship.observed_at) = 1
  AND rookery_read_is_uuid(relationship.supporting_observation_id) = 1
  AND CASE
      WHEN EXISTS (
...
WHERE rookery_read_is_uuid(binding.binding_id) = 1
  AND rookery_read_is_contract_key(binding.adapter_key) = 1
  AND rookery_read_is_text(binding.adapter_version, 64) = 1
  AND binding.enabled IN (0, 1)
  AND rookery_read_is_binding_settings(binding.non_secret_settings_json) = 1
  AND (
      binding.secret_reference IS NULL
      OR rookery_read_is_text(binding.secret_reference, 1024) = 1
  )
  AND rookery_read_is_timestamp(binding.record_created_at) = 1
  AND rookery_read_is_timestamp(binding.record_updated_at) = 1
  AND rookery_read_timestamp_order_is_valid(
...
WHERE rookery_read_is_uuid(capability.capability_binding_id) = 1
  AND rookery_read_is_contract_key(capability.capability_key) = 1
  AND rookery_read_is_text(capability.capability_version, 64) = 1
  AND capability.operation_class = 'observe'
  AND rookery_read_is_json_array(capability.target_kinds_json) = 1
  AND rookery_read_is_json_array(capability.produced_facets_json) = 1
  AND capability.enabled IN (0, 1)
  AND typeof(capability.selection_priority) = 'integer'
  AND rookery_read_is_json_array(capability.collateral_effects_json) = 1
  AND rookery_read_is_json_array(capability.mitigations_json) = 1
  AND rookery_read_is_json_array(capability.coverage_policies_json) = 1
  AND capability.coverage_policy_initialized IN (0, 1)
  AND rookery_read_is_capability(
  ...
  AND rookery_read_is_timestamp(capability.record_created_at) = 1
  AND rookery_read_is_timestamp(capability.record_updated_at) = 1
  AND rookery_read_timestamp_order_is_valid(
...
WHERE rookery_read_is_uuid(scope.scope_id) = 1
  AND (
      scope.object_id IS NULL
      OR (
          rookery_read_is_uuid(scope.object_id) = 1
          AND EXISTS (
  ...
  AND rookery_read_is_contract_key(scope.object_type) = 1
  AND scope.enabled IN (0, 1)
  AND rookery_read_is_text(scope.display_name, 1024) = 1
  AND rookery_read_is_timestamp(scope.record_created_at) = 1
  AND rookery_read_is_timestamp(scope.record_updated_at) = 1
  AND rookery_read_timestamp_order_is_valid(
...
WHERE rookery_read_is_contract_key(identity.system_kind) = 1
  AND rookery_read_is_config_id(identity.config_id) = 1
  AND rookery_read_is_authority_key(identity.authority_key) = 1
  AND rookery_read_is_timestamp(identity.record_created_at) = 1
  AND rookery_read_is_timestamp(identity.record_updated_at) = 1
  AND rookery_read_timestamp_order_is_valid(
  ...
  AND EXISTS (
      ...
      WHERE binding.system_id = identity.system_id
        AND rookery_read_identity_matches_binding(
