...
Expected configuration, local-state, and dependency failures return exit 1 with bounded terminal-safe diagnostics on stderr. Invalid usage remains argparse exit 2, bounded `run-once` incompletion is exit 3, and operator interruption is exit 130 without a traceback. Successful command data, including `fingerprint-profile` and `authority-list`, is written to stdout; informational diagnostics remain on stderr.
